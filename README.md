...
class Scalene(triangle.Triangle):#Subclase
    def __init__(self, is_regular: bool, vertices: list = [triangle.shape.point.Point], edges: list = [triangle.shape.line.Line]):
        super().__init__(is_regular, vertices, edges)#Llamando atributos y métodos de la clase base
...
import paquete.shape as shape
import paquete.triangle as triangle
import paquete.isosceles as isosceles
import paquete.equilateral as equilateral
import paquete.scalene as scalene
import paquete.rectangle as rectangle
import paquete.square as square


def main():
    is_regular_square = True
    vertices_square = [point.Point(0,0), point.Point(7,0), point.Point(7,7), point.Point(0,7)]
    edges_square = [line.Line(point.Point(0,0), point.Point(7,0)), line.Line(point.Point(7,0), point.Point(7,7)), line.Line(point.Point(7,0), point.Point(0,7)), line.Line(point.Point(0,7), point.Point(0,0))]

    shape_square = square.Square(is_regular_square, vertices_square, edges_square)
    print(shape_square.compute_area())
    print(shape_square.compute_perimeter())
    print(shape_square.compute_inner_angles())

    is_regular_triangle = False
    vertices_triangle = [point.Point(-1,0), point.Point(2,9), point.Point(4,1)]
    edges_triangle = [line.Line(point.Point(-1,0), point.Point(4,1)), line.Line(point.Point(4,1), point.Point(2,9)), line.Line(point.Point(2,9), point.Point(-1,0))]
    shape_triangle = scalene.Scalene(is_regular_triangle, vertices_triangle, edges_triangle)
    print(shape_triangle.compute_area())
    print(shape_triangle.compute_perimeter())
    print(shape_triangle.compute_inner_angles())
    
    is_regular_rectangle = False
    vertices_rectangle = [point.Point(-3,-5), point.Point(-3,-2), point.Point(5,-2),point.Point(5,-5)]
    edges_rectangle = [line.Line(point.Point(-3,-2), point.Point(-3,-5)), line.Line(point.Point(-3,-5), point.Point(5,-5)), line.Line(point.Point(5,-5), point.Point(5,-2)), line.Line(point.Point(5,-2), point.Point(-3,-2))]

    shape_rectangle = rectangle.Rectangle(is_regular_rectangle, vertices_rectangle, edges_rectangle)
    print(shape_rectangle.compute_area())
    print(shape_rectangle.compute_perimeter())
    print(shape_rectangle.compute_inner_angles())
